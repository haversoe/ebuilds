...
RDEPEND="
	...
	>=dev-qt/qtwebsockets-5.15.7:=
	sudo? ( lxqt-base/lxqt-sudo )"
DEPEND="${RDEPEND}"

# S=${WORKDIR}/${PN}-v${PV}-amd64-arch
S="${WORKDIR}/${PN}-v${PV}-build.218.1+gfd1129b-amd64-arch/${PN}-v${PV}-build.218.1+gfd1129b-amd64-arch"
# S=${WORKDIR}/${PN}-v${PV}-amd64-arch

src_unpack() {
	default
	# cd "${S}" || die "Couldn't cd into the source directory ${S}"
	cd "${WORKDIR}/${PN}-v${PV}-build.218.1+gfd1129b-amd64-arch" || die "Couldn't cd into the source directory ${S}"
	# TODO: fix x86 installation :(
	# tar xpf ${PN}-v${PV}-amd64-arch.tar.gz || die "tar failed"
	unpack ./${PN}-v${PV}-build.218.1+gfd1129b-amd64-arch.tar || die "tar failed"
}
